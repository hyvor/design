...
<Table columns="2fr 2fr 3fr">
  ...
  <TableRow>
    ...
    <div>The color of the loader circle.</div>
  </TableRow>

  <TableRow>
    <div><code>colorTrack</code></div>
    <div><code>var(--accent-lightest)</code></div>
    <div>The color of the loader circle track.</div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div>Inverts the loader colors.</div>
  </TableRow>

  <TableRow>
    <div><code>block</code></div>
    <div><code>false</code></div>
    <div>If true, the loader will be displayed as a block element.</div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div><code>false</code></div>
    <div>
      If true, the loader will take up the full width and height of its parent.
      The loader and the message will be centered.
    </div></TableRow
  >
  ...
  <TableRow>
    ...
    <div>
      Padding around the loader wrap. Can be a number in pixels or one of the
      following strings:
...
<p>
  You can quickly invert the colors of the loader by setting the <code
    >invert</code
  > prop to true. This is useful in cases where the loader is displayed on a dark
  background, for example in a button with the accent color.
</p>
...
<p>
  You can display the loader as a block element by setting the <code>block</code
...
<CodeResult>
  <Loader block>Loading...</Loader>
</CodeResult>

<p>
  You can set <code>padding={"{number}"}</code> or
  <code>padding="small|medium|large"</code>
  to add padding to the loader. By default, <code>padding="medium"</code> is used.
</p>

<h3 id="full loader">Full Loader</h3>

<p>
  You can set <code>full</code> prop to true to make the loader take up the full
  width and height of its parent. Padding is not applied in this case.
</p>
...
<CodeResult>
  <div style="height: 200px">
    <Loader full>Full Loader (200px height, centered)</Loader>
  </div>
</CodeResult>

<h3 id="inline-with-message">Inline Loader with Message</h3>
...
<p>
  You can set the <code>state</code> prop to <code>loading</code>,
  <code>success</code>
  or <code>error</code> to display the corresponding loaderState
</p>
...
<p>
  You will often need to display a button with a loader conditionally. The <code
    >LoadButton</code
  > component can be used for this purpose. One common use case is to display a button
  to load more items in a list.
</p>
