...
<p>
  Split controls are commonly used in HYVOR for settings and configuration
  options. They are a combination of a label/caption (left side) and a control
  (right side) such as a text input, checkbox, or select. On mobile, the
  label/caption is displayed above the control.
</p>
...
<Table columns="1fr 3fr">
  ...
  <TableRow>
    ...
    <div>
      The caption to display on the left side of the split control. This is
      displayed below the label, and is usually used to provide more information
      about the control.
    </div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div>
      Set flex values of the left and right sides of the split control. The
      default value is <code>[1,2]</code>.
    </div>
  </TableRow>
</Table>
...
<Table columns="1fr 3fr">
  ...
  <TableRow>
    ...
    <div>
      To nest split controls inside of other split controls. See <a
        href="#nested">Nested Split Controls</a
      > below.
    </div>
  </TableRow>
</Table>
...
<p>
  You can use slots for the label and caption if you need to add more complex
  content. <code>&lt;Label&gt;</code> and <code>&lt;Caption&gt;</code>
  components (see <a href="/form-control">Form Control</a>) or any other
  element/component can be used in the slots.
</p>
...
<p>
  You can nest split controls inside of other split controls by adding nested
  content inside the <code>nested</code> slot. The parent split control may or may
  not have a control on the right side. The nested split controls will be indented.
</p>
...
<p>
  You can set the the flex values for the left and right sides of the split
  control using the <code>flex</code> property. The default value is
  <code>[1,2]</code>.
</p>
...
<p>
  You can set the split control to display as a column using the <code
    >column</code
  > property.
</p>
