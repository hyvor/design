...
<p>
  Dropdowns are used to show a list of options when a user clicks on a button.
  Use the <a href="action-list">Action List</a> component to show the actions in
  the dropdown.
</p>
...
<Table columns="2fr 2fr 3fr">
  ...
  <TableRow>
    ...
    <div>
      Whether the dropdown should be closed when the user clicks outside of the
      dropdown.
    </div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div>
      Whether the dropdown should be shown or not. Use this property to control
      the dropdown.
    </div>
  </TableRow>
...
<p>
  ...
  <code>closeOnOutsideClick={false}</code> properties for demonstration purposes.
  However, in most cases, they are not needed.
</p>
...
<p>
  Here's a little more complex example. The key is to set <code
    >selection="single"</code
  >
...
<p>
  Here is an example using <code>{"<ActionListGroup>"}</code> and other slots in
  the <a href="action-list">Action List</a> component.
</p>
...
<CodeResult>
  <SplitControl label="Position" caption="Position of the dropdown">
    <InputGroup>
      ...
      <Radio name="paPosition" value="bottom" bind:group={paPosition}
        >Bottom</Radio
      >
      <Radio name="paPosition" value="left" bind:group={paPosition}>Left</Radio>
      <Radio name="paPosition" value="right" bind:group={paPosition}
        >Right</Radio
      >
    </InputGroup>
  </SplitControl>
  ...
  <div
    style="margin-top:120px; left:auto; margin-bottom:120px; text-align: center;"
  >
    <Dropdown
      bind:show={s1}
