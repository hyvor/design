...
<p>
  Buttons are used to trigger an action or event, such as submitting a form,
...
<Table columns="2fr 2fr 3fr">
  ...
  <TableRow>
    ...
    <div>Whether the button should be displayed as a block element.</div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div>
      The alignment of the content when using <code>block</code>. Can be
      <code>start</code>
      or <code>center</code>
    </div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div>
      The element to render the button as. Can be <code>button</code> or
      <code>a</code>.
    </div>
  </TableRow>
...
<CodeResult
  style="display:flex;flex-direction:column;gap:15px;align-items:flex-start"
>
  <Button size="x-small">X-Small Button</Button>
  <Button size="small">Small Button</Button>
  <Button size="medium">Medium Button</Button>
...
<ul>
  <li>
    ...
    <CodeBlock code={`<Button>Search</Button>`} />
    <CodeResult>
  ...
  <li>
    <code>action</code> - Placed after the content, locked to the right side of
    the button. This is useful with the <code>block</code> property.
...
<p>
  You can use the <code>disabled</code> attribute to disable a button.
</p>
<CodeBlock
  code={`
