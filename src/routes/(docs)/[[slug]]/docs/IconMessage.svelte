...
<p>
  Icon Message is a component that can be used to show a message with an icon in
...
<FormControl>
  <div style="margin-bottom: 8px;font-size:14px;color:var(--text-light);">
    Choose type:
  </div>
...
<p>
  You can use custom icons and messages with Icon Message, as well as custom
  padding, colors, and sizes.
</p>
...
<p>
  Icon Message has two templates: <code>empty</code> and <code>error</code>. You
  can use them by setting <code>empty</code> or <code>error</code> prop to
  <code>true</code>.
</p>
