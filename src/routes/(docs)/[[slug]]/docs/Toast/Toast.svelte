...
<p>
  Toasts are used to show a notification to the user. They are implemented using
  Svelte stores. The <a href="/#install"><code>{"<Base>"}</code> component</a> sets
  up the store and the wrapper components to show the toast.
</p>
...
<p>
  In all toast-creating functions, you can pass an object as the second
  parameter to override options.
</p>
...
<p>
  You can also pass a svelte component as the message. A <code>toast</code> prop
  will be passed to the component. You can use it to close the toast.
</p>
...
<p>
  You can show a loading toast by calling the <code>toast.loading()</code> function.
  The toast will not be auto-closed. You may update its type by calling a toast-creating
  function with the same toast id.
</p>
