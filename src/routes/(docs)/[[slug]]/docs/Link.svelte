...
<p>
  This component is a wrapper around the <code>&lt;a&gt;</code> tag. By default,
  we remove all styles from the default <code>{"<a>"}</code> tag. You can use this
  component to create links with styles like link color.
</p>
...
<Table columns="2fr 2fr 3fr">
  ...
  <TableRow>
    <div><code>underline</code></div>
    <div><code>true</code></div>
    <div>Whether the link should be underlined.</div>
  </TableRow>
</Table>

<p>
  All additional props you pass to this component will be passed to the <code
    >{"<a>"}</code
  > tag.
</p>
...
<div class="no-link-color">
  <CodeResult gap={6}>
    ...
    <Link href="/" underline={false} target="_blank"
      >Open<IconBoxArrowUpRight slot="end" /></Link
    >
  </CodeResult>
</div>
