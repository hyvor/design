...
<ul>
    ...
    <li>
        The other languages can be loaded dynamically using the <code
            >loader</code
        > property.
    </li>
</ul>
...
<p>
    ...
    <a href="https://formatjs.io/docs/intl-messageformat">
        intl-messageformat
...
<p>
    The <code>plural</code> format is used to handle pluralization. The value is
    expected to be a number. These plural categories are supported:
</p>
...
<p>
    You can pass components as parameters to the strings. In the following code, <code
        >{`<a></a>`}</code
    >
    is a placeholder for a component. The content within it ("this article") will
    be sent to the component as the <code>children</code> prop.
</p>
