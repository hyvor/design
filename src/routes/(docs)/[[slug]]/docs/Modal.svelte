...
<p>
  The <code>{"<Modal>"}</code> component is used to display an interactive modal
  dialog.
</p>
...
<Table columns="2fr 2fr 3fr">
  ...
  <TableRow>
    ...
    <div>If true, the modal will be displayed.</div>
  </TableRow>

  <TableRow>
    <div><code>title</code></div>
    <div></div>
    <div>The title of the modal.</div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div>
      An id to be used in the aria-labelledby and aria-describedby attributes.
      Only required if you are using multiple modals at the same time.
    </div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div>
      <code>alertdialog</code> or <code>dialog</code>. Use the latter if the
      modal is not an alert.
    </div>
  </TableRow>
  ...
  <TableRow>
    ...
    <div>Whether to close the modal when clicking outside of it.</div>
  </TableRow>
...
<Table columns="2fr 3fr">
  ...
  <TableRow>
    ...
    <div>The content/body of the modal.</div>
  </TableRow>

  <TableRow>
    <div><code>title</code></div>
    <div>Override the title property of the modal.</div>
  </TableRow>

  <TableRow>
    <div><code>footer</code></div>
    <div>The footer of the modal.</div>
  </TableRow>
</Table>
...
<CodeResult>
  ...
  <Modal title="Confirm to delete" bind:show={show1}>
    Please confirm that you want to delete this item. This action cannot be
    undone.

    <div slot="footer">
      <Button variant="invisible" on:click={() => (show1 = false)}
        >Cancel</Button
      >
      <Button
        color="red"
...
<p>
  Set the <code>size</code> attribute to <code>small</code>, <code>medium</code>
  or <code>large</code> to change the size of the modal.
</p>
...
<p>
  If a modal grows larger than the screen, it will be scrollable. The inner
  content will be aligned to the top of the modal.
</p>
...
<p>
  You can use the <code>loading</code> property to display a loading spinner in the
  modal. It accepts a string or a boolean. If a string is passed, it will be displayed
  as the loading text. This is the recommended way to display a loading state in
  a modal, due to the following issues with other methods:
</p>
...
<Table columns="1fr 1fr">
  ...
  <TableRow>
    <div>Loading toast</div>
    <div>The user can still interact with the modal.</div>
  </TableRow>
  <TableRow>
    <div>Closing the modal and showing a toast</div>
    <div>The user's input will be lost in case of an error.</div>
  </TableRow>
  <TableRow>
    <div>Show a loader in the button manually</div>
    <div>
...
<Modal title="Loading Modal" bind:show={show7} loading={loading7}>
  Click the button below to start loading. It will show a loading spinner
  covering the modal. You can use this to prevent the user from interacting with
...
<p>
  Setting the <code>loading</code> property will disable the close button, escape
  key, and the outside click to close the modal. Therefore, you should handle both
  success and error cases manually.
</p>

<h3 id="modal-footer">Modal Footer</h3>

<p>
  Instead of manually setting the footer slot, you can set the <code
    >footer</code
  > property of the modal to display a common footer with a cancel and a confirm
  button.
</p>
...
<p>
  Confirm modals are used frequently in web applications to confirm an action.
  You can use the <code>confirm</code> function to display a confirm modal.
</p>
...
<p>
  First, set the <code>autoClose</code> property to <code>false</code> to
  prevent the modal from closing when the confirm button is clicked. Then, use
  the <code>loading</code> method of the returned object to set the
  <a href="#loading">modal loading state</a>. Finally, close the modal manually
