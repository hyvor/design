...
<p>
  Text inputs are used to collect text from the user. They are commonly used in
...
<Table columns="1fr 1fr 2fr">
  ...
  <TableRow>
    ...
    <div>
      The state of the text input. This will change the border color of the text
      input. States can be one of:
      <ul>
  ...
  <TableRow>
    ...
    <div>
      If <code>true</code>, the text input will take the full width of its
      parent.
    </div>
  </TableRow>
...
<p>
  The ending slot is useful with the <a href="/loader">Loader</a> component to indicate
  a loading state, for example in a search input.
</p>
