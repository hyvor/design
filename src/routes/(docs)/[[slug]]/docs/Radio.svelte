...
<p>
  <code>{"<Radio>"}</code> is a form control that allows users to select one
  option from a list of options. All properties are sent directly to the
  underlying HTML <code>{'<input type="radio">'}</code> element.
...
<CodeResult>
  <SplitControl label="Hosted at" caption="Where to host your blog">
    <InputGroup>
      <Radio name="hosted-at" value="subdomain" bind:group={opt2}
        >Subdomain</Radio
      >
      <Radio name="hosted-at" value="domain" bind:group={opt2}
        >Custom Domain</Radio
      >
